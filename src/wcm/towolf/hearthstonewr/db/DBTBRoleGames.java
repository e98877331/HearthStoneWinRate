...
public class DBTBRoleGames {
	...
	public ArrayList<AddDateRoleGame> getAllGameForRoleID(int id) {
		ArrayList<AddDateRoleGame> retData = new ArrayList<AddDateRoleGame>();

		String fields[] = { "Game_ID", "Role_ID", "Role_Type_Num", "Is_Win","Add_Date" };

		String rule[] = { Integer.toString(id) };
		Cursor c = mDBHelper.select(TABLE, fields, "Role_ID = ?", rule, null,
				null, null);
		while (c.moveToNext()) {

			// TODO fix win rate;
			AddDateRoleGame rg = new AddDateRoleGame(c.getInt(0), c.getInt(1), c.getInt(2),
