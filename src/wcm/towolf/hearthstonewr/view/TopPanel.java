...
public class TopPanel extends RelativeLayout{
	...
	public TopPanel(Context context,RatioFixer rf) {
		...
		this.addView(new LabelText(context,R.string.detail_text_win),rf.getLayoutParam(lbw, lbh, 0, 0));
		this.addView(new LabelText(context,R.string.detail_text_lose),rf.getLayoutParam(lbw, lbh, lbw, 0));
		this.addView(new LabelText(context,R.string.detail_text_total),rf.getLayoutParam(lbw, lbh, 2*lbw, 0));
		this.addView(new LabelText(context,R.string.detail_list_winrate),rf.getLayoutParam(lbw, lbh, 3*lbw, 0));
    ...
    public void update(ArrayList<RoleData> data)
    {
    	int total =0 , win = 0;
    	
       for(int i = 0 ; i < data.size();i++)
       {
    	   RoleData rd = data.get(i);
